...
serve(async (req) => {
  ...
  try {
    ...
    const ownerSubject = `Új HAIRPORT by Timi foglalás - ${booking.customer_name}`;
    const customerSubject = "HAIRPORT by Timi foglalásod beérkezett";
...
function calendarEvent(adatok: {
  ...
  return [
    ...
    "BEGIN:VEVENT",
    `UID:${icsText(adatok.id)}@hairport.local`,
